...
Each of the 5 importable entities (Suppliers, Products, Purchase Orders, Users, Roles) has a "Download example template" button inside its Import dialog — an .xlsx with the recognized column headers and one filled-in example row, generated by `GET /api/imports/template?model=...`.
